...
from .agent_endpoint import SubprocessAgentEndpoint
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run G2.3 reference agent-process isolation against the "
            "G2.2 sealed private holdout pack"
        )
    )
    parser.add_argument(
        "--pack",
            ...
            rows.append(
                {
                    ...
                    "agent_transcript_valid": (
                        isolated.agent_transcript_valid
                    ),
    ...
    summary = {
        "format": "proof-gym-isolated-agent-private-pack-reference-run-v1",
        "evaluation_scope": (
            "reference_agent_process_isolation_conformance_only"
        ),
        ...
        "agent_transport": "proof-gym-agent-jsonl-v1",
